---
title: AlmaLinux
description: AlmaLinux 9 / 10 x86_64 管理対象ガイド — dnf バックエンド。
---

AlmaLinux 9 と AlmaLinux 10 の x86_64 はサポートされ、受入検証済みの
管理対象です。どちらも **dnf** パッケージバックエンドを使います。

**受入基準環境:** AlmaLinux 9.8 x86_64 および AlmaLinux 10.2 x86_64 —
実際の SSH と `sudo -n` 越しに、パッケージのインストール/削除/冪等性、
file、service、command リソースについて検証済みです。
AlmaLinux 9/10 は対応するversion lineであり、9.8 と 10.2 が検証済みの
...
`/etc/os-release` の `ID=almalinux`（RHEL ファミリ）が dnf バックエンドを
...
- AlmaLinux 9.8 と 10.2 が受入基準環境です。他のマイナーリリースは
  同じインターフェースを共有しますが、個別には検証されていません。
