...
description: RHEL 9 and 10 x86_64 managed-target guide — dnf backend.
---

RHEL 9 and RHEL 10 x86_64 are supported, acceptance-tested managed targets.
Both use the **dnf** package backend.

**Acceptance references:** RHEL 9.8 x86_64 and RHEL 10.2 x86_64 — verified
for package install/remove/idempotency, file, service, and command resources
over real SSH with `sudo -n`, including package payload download through
natively authenticated repositories. RHEL 9/10 are the supported version
lines; 9.8 and 10.2 are the verified references, not independent evidence
for every other minor release.
...
Standard configured DNF repositories must be functional. Repository
transport and authentication are delegated to the native `dnf`/librepo
stack — including cloud images whose repositories are already entitled by
the provider's repository infrastructure. Sinter does not handle repository
credentials itself, and no Sinter-specific repository configuration is
required.
...
`/etc/os-release` `ID=rhel` (RHEL family) selects the dnf backend. Unknown or
unsupported platforms are capability errors — Sinter does not guess a
backend.

## Limitations

- Only x86_64 is validated; do not assume aarch64 support.
- RHEL 9.8 and 10.2 are the acceptance references — other minors share the
  same interfaces but are not individually verified.
